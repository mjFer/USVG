/**
 * Codigo Basado en el Polygon tools original de Farseer Physics
 * 
 */
...
namespace USVG 
{
	...
	public static class PolygonTools {
		...
		{
			Vector2[] vertices = CreateRectangle(hx, hy);
			...
			// Transform vertices
			for (int i = 0; i < 4; ++i) {
				vertices[i] = newRotation * ((Vector3)vertices[i] - (Vector3)center) + (Vector3)center;
			}
		...
		/// <param name="segments">The number of segments to subdivide the edges.</param>
		/// <returns></returns>
		public static Vector2[] CreateRoundedRectangle(float width, float height, float xRadius, float yRadius,
													  int segments)
		{
			if (yRadius > height / 2 || xRadius > width / 2)
				throw new Exception("Rounding amount can't be more than half the height and width respectively.");
		...
		public static Vector2[] CreateLine(Vector2 start, Vector2 end)
		{
		...
		/// <param name="numberOfEdges">The number of edges. The more edges, the more it resembles a circle</param>
		/// <returns></returns>
		public static Vector2[] CreateCircle(float radius, int numberOfEdges)
		{
		...
		/// <param name="numberOfEdges">The number of edges. The more edges, the more it resembles an ellipse</param>
		/// <returns></returns>
		public static Vector2[] CreateEllipse(float xRadius, float yRadius, int numberOfEdges)
		{
			List<Vector2> vertices = new List<Vector2>();
		...
		public static Vector2[] CreateArc(float radians, int sides, float radius)
		{
		...
		/// <returns></returns>
		public static Vector2[] CreateCapsule(float height, float endRadius, int edges)
		{
		...
		/// <returns></returns>
		public static Vector2[] CreateCapsule(float height, float topRadius, int topEdges, float bottomRadius,
											 int bottomEdges)
		{
			if (height <= 0)
		...
		/// <returns></returns>
		public static Vector2[] CreateGear(float radius, int numberOfTeeth, float tipPercentage, float toothHeight)
		{
			List<Vector2> vertices = new List<Vector2>();

			float stepSize = Mathf.PI*2 / numberOfTeeth;
			tipPercentage /= 100f;
		...
		///// <returns></returns>
		//public static Vector2[] CreatePolygon(uint[] data, int width)
		//{
